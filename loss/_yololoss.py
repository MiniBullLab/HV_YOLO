...
from loss.cross_entropy import *
...
class YoloLoss(nn.Module):
    ...
    def __init__(self, num_classes, anchors, anchors_mask, seen=0, coord_scale=1.0, noobject_scale=1.0,
                 object_scale=1.0, class_scale=1.0, thresh=0.5, head_idx=0, smoothLabel=False, focalLoss=False):
        super().__init__()
        ...
        if self.smoothLabel:
            self.ce = cross_entropy(reduction='sum')
        else:
    ...
    def forward(self, output, target=None, seen=None):
        ...
        if target is not None:
            ...
            if nC > 1:
                ...
                # label smooth
                if self.smoothLabel:
                    smoothL = smoothLabel(tcls, self.num_classes)
