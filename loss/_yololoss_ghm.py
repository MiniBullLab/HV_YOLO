...
from loss.ghm_loss import GHM_Loss
...
class YoloLoss(nn.Module):
    ...
    """
    def __init__(self, num_classes, anchors, anchors_mask, seen=0, coord_scale=1.0, noobject_scale=1.0, object_scale=1.0, class_scale=1.0, thresh=0.5, head_idx=0):
        super().__init__()
        ...
        self.ghm_loss = GHM_Loss(bins=30, momentum=0.5)
    ...
    def forward(self, output, target=None, seen=None):
        ...
        if target is not None:
            ...
            ghm_loss_cong_pos = 1.0 * self.object_scale * (ghm_loss(conf, tconf, conf_pos_mask)).sum()
            #loss_conf_neg = 1.0 * self.noobject_scale * (conf_neg_mask * bce(conf, tconf)).sum()
            ghm_loss_cong_neg = 1.0 * self.noobject_scale * (ghm_loss(conf, tconf, conf_neg_mask)).sum()
            self.loss_conf = ghm_loss_cong_pos +  ghm_loss_cong_neg
