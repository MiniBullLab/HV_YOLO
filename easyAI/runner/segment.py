...
from easyAI.config import segmentConfig
...
class Segmentation():
    ...
    def segment(self, input_path):
        if os.path.isdir(input_path):
            dataloader = ImagesLoader(input_path, batch_size=1, img_size=segmentConfig.imgSize)
        else:
            dataloader = VideoLoader(input_path, batch_size=1, img_size=segmentConfig.imgSize)
