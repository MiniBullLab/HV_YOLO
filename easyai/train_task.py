...
class TrainTask():
    ...
    def classify_train(self, cfg_path, gpu_id, pretrain_model_path):
        cls_train_task = ClassifyTrain(cfg_path, gpu_id)
    ...
    def detect2d_train(self, cfg_path, gpu_id, pretrain_model_path):
        det2d_train = Detection2dTrain(cfg_path, gpu_id)
    ...
    def segment_train(self, cfg_path, gpu_id, pretrain_model_path):
        seg_train = SegmentionTrain(cfg_path, gpu_id)
    ...
    def pc_classify_train(self, cfg_path, gpu_id, pretrain_model_path):
        pc_cls_train_task = PointCloudClassifyTrain(cfg_path, gpu_id)
        pc_cls_train_task.load_pretrain_model(pretrain_model_path)
...
def main():
    ...
    if options.task_name == TaskName.Classify_Task:
        ...
        train_task.pc_classify_train(options.cfg, 0, options.pretrainModel)
    print("process end!")
