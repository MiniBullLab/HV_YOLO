...
class UNetSeg(BaseModel):
    ...
    def down_layers(self):
        ...
        down4 = DownBlock(in_channels=512, out_channels=512,
                          bn_name=self.bn_name,
                          activation_name=self.activation_name)
        self.add_block_list(down4.get_name(), down4, 512)

    def up_layers(self):
        up1 = UpBlock(in_channels=1024, out_channels=256,
                      bn_name=self.bn_name,
                      activation_name=self.activation_name)
        self.add_block_list(up1.get_name(), up1, 256)

        up2 = UpBlock(in_channels=512, out_channels=128,
                      bn_name=self.bn_name,
                      activation_name=self.activation_name)
        self.add_block_list(up2.get_name(), up2, 128)

        up3 = UpBlock(in_channels=256, out_channels=64,
                      bn_name=self.bn_name,
                      activation_name=self.activation_name)
        self.add_block_list(up3.get_name(), up3, 64)

        up4 = UpBlock(in_channels=128, out_channels=64,
                      bn_name=self.bn_name,
                      activation_name=self.activation_name)
        self.add_block_list(up4.get_name(), up4, 64)
    ...
    def forward(self, x):
        ...
        for key, block in self._modules.items():
            ...
            elif UNetBlockName.UpBlock in key:
