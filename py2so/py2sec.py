...
Example:
  ...
  python py2sec.py -d test_dir -m __init__.py,setup.py,[poc/,resource/,venv/,interface/]
    ...
    def pyEncrypt(self):
        ...
        p = subprocess.Popen("python setup.py build_ext > log.log", shell=True, stderr=subprocess.PIPE)
        p.wait()
        err = p.stderr.readlines()
        if err:
            if self.py_ver == '3':
                p = subprocess.Popen("python3 setup.py build_ext > log.log", shell=True, stderr=subprocess.PIPE)
                p.wait()
            else:
    ...
    def genProject(self):
        if not os.path.exists("result/"):
            os.mkdir("result/")
        if self.root_name:
            enc_root_name = "result/%s"%self.root_name
