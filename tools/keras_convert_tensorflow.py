...
from converter.keras_models.model_factory import KerasModelFactory
...
keras_model_factory = KerasModelFactory()
...
def load_model(input_model_path, input_json_path=None, input_yaml_path=None):
    ...
    try:
        model = keras_model_factory.load_model(input_model_path, FLAGS.input_model_name)
        return model
...
class KerasConvertTensorflow():
    ...
    def __init__(self, h5_model_path, model_name):
        output_path = Path(h5_model_path).with_suffix(".pb")
        FLAGS.output_model = str(output_path)
        FLAGS.input_model = h5_model_path
        FLAGS.input_model_name = model_name
