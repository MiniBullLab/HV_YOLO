...
# Author:
"""
Ibtehaz, Nabil, and M. Sohel Rahman.
"MultiResUNet: Rethinking the U-Net architecture for multimodal biomedical image segmentation."
Neural Networks 121 (2020): 74-87.
"""

from easy_convert.keras_models.utility.base_model import BaseModel
...
def MultiResUnet3D(height, width, z, n_channels):
    ...
    up6 = concatenate([Conv3DTranspose(32 * 8, (2, 2, 2), strides=(2, 2, 2), padding='same')(mresblock5), mresblock4],
                      axis=4)
    mresblock6 = MultiResBlock(32 * 8, up6)

    up7 = concatenate([Conv3DTranspose(32 * 4, (2, 2, 2), strides=(2, 2, 2), padding='same')(mresblock6), mresblock3],
                      axis=4)
    mresblock7 = MultiResBlock(32 * 4, up7)

    up8 = concatenate([Conv3DTranspose(32 * 2, (2, 2, 2), strides=(2, 2, 2), padding='same')(mresblock7), mresblock2],
                      axis=4)
    mresblock8 = MultiResBlock(32 * 2, up8)

    up9 = concatenate([Conv3DTranspose(32, (2, 2, 2), strides=(2, 2, 2), padding='same')(mresblock8), mresblock1],
                      axis=4)
...
class MultiResUnet3D(BaseModel):
    """
            height {int} -- height of image
            width {int} -- width of image
            z {int} -- length along z axis
            n_channels {int} -- number of channels in image
    """
    def __init__(self, input_size=(80, 80, 48, 4)):
        super().__init__()
        self.input_size = input_size
        self.set_name(KerasModelName.MultiResUnet3D)
    ...
    def model_init(self):
        ...
        up6 = concatenate(
            [Conv3DTranspose(32 * 8, (2, 2, 2), strides=(2, 2, 2), padding='same')(mresblock5), mresblock4],
            axis=4)
        mresblock6 = MultiResBlock(32 * 8, up6)

        up7 = concatenate(
            [Conv3DTranspose(32 * 4, (2, 2, 2), strides=(2, 2, 2), padding='same')(mresblock6), mresblock3],
            axis=4)
        mresblock7 = MultiResBlock(32 * 4, up7)

        up8 = concatenate(
            [Conv3DTranspose(32 * 2, (2, 2, 2), strides=(2, 2, 2), padding='same')(mresblock7), mresblock2],
            axis=4)
        mresblock8 = MultiResBlock(32 * 2, up8)
...
def main():
    # Define the model
    model = MultiResUnet3D(input_size=(80, 80, 48, 4))
